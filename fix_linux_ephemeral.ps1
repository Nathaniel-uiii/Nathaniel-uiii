# Fix Linux Flutter Ephemeral Directory Lock Issue
# This script resolves the "Flutter failed to delete a directory" error for Linux builds

Write-Host ""
Write-Host "=== Fixing Linux Flutter Ephemeral Directory Lock ===" -ForegroundColor Cyan
Write-Host ""

$scriptRoot = Get-Location | Select-Object -ExpandProperty Path
$ephemeralPath = "linux\flutter\ephemeral\.plugin_symlinks"
...
if (Test-Path $fullPath) {
    ...
    if (-not $deleted) {
        ...
        Write-Host "  3. Or exclude 'linux\flutter\ephemeral' from OneDrive sync" -ForegroundColor White
        Write-Host ""
        ...
        Write-Host "  - Uncheck 'linux\flutter\ephemeral' folder" -ForegroundColor White
        exit 1
    }
} else {
...
$ephemeralDir = Join-Path $scriptRoot "linux\flutter\ephemeral"
...
Write-Host "You can now run: flutter build linux" -ForegroundColor Cyan
